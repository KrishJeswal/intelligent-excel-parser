...
from .utils import extract_unit_hint, normalize_text
...
def _load_registry(name: str) -> List[Dict]:
...
def parse_excel(file_bytes: bytes) -> ParseResponse:
    try:
        ...
        assets = _load_registry("assets.json")
        params = _load_registry("parameters.json")
        ...
        for m in mappings:
            if m.param_name is not None and m.param_name not in param_set:
                warnings.append(
                    f"Column {m.column_index}: param '{m.param_name}' not in registry; treating as unmapped."
                )
                m.param_name = None
                m.confidence = "low"
            ...
            if m.asset_name is not None and m.asset_name not in asset_set:
                ...
                if cleaned in asset_set:
                    ...
                    warnings.append(
                        f"Column {m.column_index}: asset '{m.asset_name}' not in registry; set to null."
                    )
                    m.asset_name = None

            if m.asset_name is None:
                hint = next(
                    (c.asset_hint for c in columns if c.column_index == m.column_index),
                    None,
                )
        ...
        unmapped: List[UnmappedColumn] = [
            UnmappedColumn(
                ...
                reason=(
                    mapping_by_col[c.column_index].reason
                    if c.column_index in mapping_by_col
                    else "No mapping returned by LLM."
                ),
            )
            for c in columns
            if not mapping_by_col.get(c.column_index)
            or not mapping_by_col[c.column_index].param_name
        ]
